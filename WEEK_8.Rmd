...
There are other charts as well, which are less popular to hate but, in certain situations, may obscure important information. In some cases, bar plots can hide important features of your data and might not be the most appropriate means for comparison. See the below image, for example, where the same data about two groups, green and purple, are visualised using three different methods: a histogram, which shows the green group following a normal distribution and the purple group following a heavily skewed distribution, a boxplot that shows the same, and finally a bar plot, which makes the green and purple group appear identical: 
...
All right, now you have chosen a scheme. Let's stick with sequential; only the maximum police effectiveness perceived will do! So now, how can we find out what are some good colours to use? Go to the site [http://colorbrewer2.org/](http://colorbrewer2.org/), and select sequential from the colour scheme options: 
