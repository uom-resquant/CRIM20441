...
Much of statistics is about making comparisons. Human beings are not good at sifting through large streams of data; we understand data much better when it is summarised for us. This is true for looking for patterns in both *uni*variate and *bi*variate analysis. As discussed last week in the context of univariate analysis, when presenting descriptive statistics, we often display summary statistics in one of two ways: through tables and figures. 


Tables of summary statistics are very common (we already created some of these last week) – nearly all published studies in criminology will contain a table of basic summary statistics describing their sample. However, figures offer a visually more appealing interpretation of our data, allowing people to identify trends from large amounts of information easily. This is also true for exploring relationships between two variables. In this course, we will explore methods for producing these visuals (and some tables) that can help you begin thinking about the relationships between different variables in your data. Not only do we want to be able to summarise one variable, but we also want to know if it is related to another variable. This is where the interesting questions that we can start asking are. 
...
For example, a researcher might be investigating the relationship between the class on which a passenger was travelling on the RMS Titanic (which, in case you're not familiar, was a British passenger liner that sank in the North Atlantic Ocean in 1912 after it collided with an iceberg) and whether that person survived or not. The two variables would be class (1st, 2nd, 3rd, or crew) and survived (yes/no). The question is, "Is there a significant relationship between class of passenger and survival?". In this course, we don't learn how to answer this question with  *inferential statistics*. Instead, we only begin to explore this question (and other comparison questions) using *descriptive statistics*. If you are unsure about the difference, ask now or consult your readings from the first week. 


### Surviving the Titanic


Let's return to our question about survival on the Titanic. We want to determine the relationship between the variable indicating the class of a passenger and the variable indicating survival. First, we need some data. Data about the fate of the RMS Titanic's passengers is actually available as open data, allowing us to explore any questions we may have about it. [You can read a bit about the data here](https://www.statisticalconsultants.co.nz/blog/titanic-survival-data.html). In this instance, though, you can download the data from Canvas. As always, it's in your Learning Materials for the week, under Module 3 > data for labs and homework. It is the one labelled "Titanic survivors data". Under it, you will see a link to titanic3.xlsx. Save this file in the working directory you set up for this course. 


Once you have downloaded the data, open it in Excel. You should see your variables in your columns and your observations in rows. Each observation is of one passenger who was on board the ship. The variable "Class" tells whether the passenger was travelling 1st class, 2nd class, 3rd class, or as a member of the crew. The variable "Survived" tells whether that person survived the RMS Titanic's collision with the iceberg or not. 


We might be interested in examining the rate of survival among passengers who were travelling on various class tickets. For example, we might have seen the 1997 classic film [Titanic](http://www.imdb.com/title/tt0120338/) by James Cameron and wonder - was priority given to first-class passengers when boarding the lifeboats, and did that result in them being more likely to survive? Well, to be able to answer questions like this, we would need to compare the survival (yes/no) between the class (1st/2nd/3rd/crew) variables. Both of these are categorical variables, so to discuss their relationship, we will need to create a **crosstab**.

#### Activity: Crosstab of survivors by class

To do this, we return to our trusty friend, the pivot table, again. But this time, instead of using it to summarise one variable, we can use it to summarise the relationship between two variables. In the same way that the GIF above illustrates with gender and the tips example, what this does is count the frequency of the combination of each category. But let's see how to do this. 
...
![](imgs/week3/click_pivot.png)




This will open a pop-up window. Ensure that you select 'New worksheet' when it asks where your pivot table should be placed, and then click OK, just as you did last week. When you click OK, Excel should take you to the new worksheet, where it has set up a pivot table for you, ready for you to enter your data, exactly the same as last week. Remember, if the toolbar doesn't appear or ever disappears, to summon it, you have to do one simple step, which is to click *anywhere* inside the pivot table area.


![](imgs/week3/pivot_shell.png)
...
Luckily, there is only one more step we need to take to achieve this, which is to introduce the Survived variable into our pivot table. To do this, drag the "Survived" variable into the "Column Labels" box (this box may also be called "Columns"). Once you've done that, you should see your frequency table appear:



![](imgs/week3/crosstabl_pivot.png)




You can see that we have a frequency table that has the variable of *Class* as the rows and the variable of *Survived* across the columns. Note that this differs from a data set, where each variable would be a column and each row an observation. This is a frequency table (also known as a cross-tab or contingency table), where both the rows and columns are composed of the values of each variable. Variable 1 in our rows is "Class" and has the values First, Second, Third, and Crew, while Variable 2 is "Survived" and has the values Yes or No.  

#### Activity: Don't forget the levels of measurement!
...
No, it is not because Excel puts the values of the class variable in alphabetical order; it doesn't know what the actual order is. The good news is that we can fix this. 
...
Let's consider the passengers of the Titanic - we can arrange them according to the amount of money they paid to be on board. The most money was spent by those travelling First class, followed by Second and Third. We can also think that the least money was paid by the Crew, who were instead given wages to work on board the ship. So the order would be First > Second > Third > Crew. 


Now, we can move around the values by right-clicking on their cells, choosing "Move", and then selecting the appropriate place to move the value.

![](imgs/week3/rearrange_crew.png)

In this case, we are quite lucky, and we only need to move 'Crew' to the end of the list to achieve our ideal order. 
You can rearrange this manually and copy it over into a new sheet. 



***Note: For older versions of Excel, the "Move" option may not be available. ******
<style>
details {
...
<details>
<summary><i>Click here if the "Move option" is not available!</i></summary>
...
![](imgs/week3/titanic_row_total_calc.png)
...
![](imgs/week3/surv_perc_cols.png)


Now, you can enter the calculation as a formula for each row. Remember, for something to be a formula, you will start with `=` and then follow with your equation. You can either type this directly in the cell you want the answers, or you select the cell and then go to the top (just above where all the columns are named - A,B,C etc), and type your formula in the formula bar. Here, the equation will be:
...
But in any case, for the cell that represents the row percentage for the yes survived and first-class combination, you should have the following formula: 
...
![](imgs/week3/perc_surv_value.png)
...
![](imgs/week3/one_dec_row_perc.png)



### Activity: Visualising the relationship - Stacked bar charts and conditional formatting
...
![](imgs/week3/stacked_col.png)


Once you click on that, our chart will appear: 

![](imgs/week3/stacked_col_2.png)
...
![](imgs/week3/horizontalcataxislabels.png)

For some others, it might look like this:

![](imgs/week3/stacked_col_4.png)
...
![](imgs/week3/stacked_col_perc.png)
...
Have a look at this Tweet, which shows what this scale might look like for someone who has this particular flavour of colourblindness: [https://x.com/MartinRGalpin/status/1317552302207737856](https://x.com/MartinRGalpin/status/1317552302207737856)

![](imgs/week3/colb_condf.png)
...
If you want to have a look at this data set, you can access it through the UK data service website [here](https://discover.ukdataservice.ac.uk/catalogue/?sn=5807). However, to make things easier, we have selected just a few variables for you here and uploaded this subset onto Canvas. You can go to Canvas (Modules -> Learning Materials week 3 -> data for labs and homework) and download the arrestees_subset file. Once downloaded onto your PC, open it up with Excel. 
...
For example, [joy riding is a crime traditionally associated with younger people](http://www.tandfonline.com/doi/abs/10.1080/10683160512331316343). Joyriding is associated with 'theft of vehicle', which appears in our data as a possible value for the "Reason for Arrest" variable. So, we might expect the distribution for age (a numeric variable) to be different for those arrested for 'theft of vehicle' than for a crime that might be associated with an older demographic. Any idea what crime types those could be? 

### Activity: Age of offenders
...
![](imgs/week3/reason_arr_values.png)
...
So, answer the following questions for me, without peeking ahead or checking the data, just in terms of your perceptions of the relationship between age and types of offences committed:
...
Well, on the right-hand side of the item "Sum of Age" in the Values box, there is a little letter "i". This is on a Mac, but on a PC, it might be a downward arrow. Whatever it is, click on it, and a new pop-up window will appear. 


![](imgs/week3/change_from_sum.png)
...
![](imgs/week3/windows_pivot_change_sum.gif)


In this case, select "Value Field Settings..." - this is the same as the steps we followed last week to add the column of percentages for our univariate frequency table. Then the pop-up that appears will look roughly the same, with the option to change the Field Name (as "Custom Name") and the Summarize by (as "Summarize Value field by By"). 



In the new pop-up window, you can see that you have a list of options in the window under the description that says "Summarize value field by". You can see that initially (by default), this is set to sum: 


![](imgs/week3/set_to_sum.png)


Change this to select "Average" like so: 


![](imgs/week3/set_to_avg.png)
...
### Activity: Adding a column for median
...
For example, if we are calculating the median age for assault for every line, we first need to make sure that the value in the *reason for arrest* column is equal to *assault*. If it is, then we use that person's age to calculate the median age for theft of a vehicle. If it is not, we do not. 
...
![](imgs/week3/med_excel_equ.png)



When you are comfortable with this formula, recreate it / type it over into the cell in the new "median" column you've created in your table and hit Ctrl+Shift+Enter. 
...
![](imgs/week3/curly_brack_formula.png)
...
![](imgs/week3/final_table_with_median.png)
...
What would our inclination be? Clearly, if the person has their first arrest earlier on in life, they have a lot more time to also have some more arrests. Or they might have an early arrest and participate in some sort of intervention whereby they turn their life around and never offend again. In other words, we are thinking that either **as the age of 1st arrest decreases, the number of arrests will increase**, or we think that **as the age of 1st arrest decreases, the number of arrests will also decrease**. These two scenarios describe two different kinds of relationships: a positive or a negative relationship. In either scenario, we assume that as one goes up, the other one goes up, or as one goes up, the other one goes down, but this takes a linear relationship. If you imagine one numeric variable across the x-axis and another across the y-axis, a positive and a negative relationship would look something like this: 

![](imgs/week3/pos_neg.png)
...
In summarising the relationship between two quantitative variables, we need to consider:
...
### Activity: Does age at first arrest correlate with the total number of arrests?
...
![](imgs/week3/scatter_4.png) 


And then, on the pop-up, there is an option to switch row/column: 


![](imgs/week3/scatter_5.png)
...
![](imgs/week3/scatter_6.png)



![](imgs/week3/scatter_7.png)
...
![](imgs/week3/scatter_8.png)



You can also stylise your scatterplot to make it look how you are happiest with it. [Here](https://blog.hubspot.com/marketing/excel-graph-tricks-list) and [here](https://www.goskills.com/Excel/Resources/How-to-make-a-scatter-plot-in-Excel) you can find some tips on making your graphs look pretty. Here's ours:


![](imgs/week3/scatter_9.png)
...
<details>
<summary><i><b><span style="color: red;">Click here</span></b> if you can't see the red line</i></summary>

The newly created trendline may appear in the same color as your scatterplot points, making it hard to see. To make the line stand out, change its color to red.

Double-click on the graph — the "Format Chart Area" panel will open on the right side of the screen.

Under "Chart Options", select Trendline 1.

Change the color to red and set the dash type to solid.

You should now have a graph that matches the example shown below.




![](imgs/week3/scatter_red_line.gif)
...
So far, we have visualised relationships between two quantitative variables using scatterplots and described the overall pattern of a relationship by considering its direction, form, and strength. We noted that assessing the strength of a relationship just by looking at the scatterplot is quite difficult, and therefore, we need to supplement the scatterplot with some kind of numerical measure that will help us assess the strength. This is what inferential statistics like correlation coefficients will be able to do, but that's for the future. 
...
![](imgs/week3/supr_corr.png)

([and you can see more strange correlations here](http://www.tylervigen.com/spurious-correlations))


But in other cases, it's much more subtle. And it can happen between variables that you can weave together a very nice story or explanation as to why they should be related. No matter what, you should always think about these examples when you are looking at correlations. We will talk later about study design, especially in relation to being able to infer causality, but do make a note now to keep in mind in the future that *correlation does not prove causation*. 
