...
```{r, echo = FALSE}
knitr::include_graphics("imgs/week2/oitnb_dance.gif")
```

Ready? OK, we can tell you now.
<style>
details {
...
So, to find this data set, just go to Canvas, navigate to the learning materials for week 2, click on `Data for the lab and the homework tasks` and then download the file `gmp_crimes.xlsx`. Remember to place it into your clear and logical folder structure! Once you have saved it, open the file using Excel.
...
![](imgs/week2/open_gmp_crimes.png)
...
Did you decide that crime type is a nominal variable? If you did - nice work! Indeed, it would be very hard to find a meaningful order for the categories in there. You could order alphabetically, but remember that it is not *meaningful*. You cannot agree on what comes first in the same way that you would be able to on a scale of *strongly disagree* to *strongly agree*. Therefore, it is nominal. This part also does matter, but we will return to why later. 
...
### Activity: Looking at frequency

For now, we will use a pivot table to create a frequency table of the crime type variable in the GMP crimes data. To do this, go to your gmp_crimes data set, which is opened up in Excel. Download the data from Canvas. If it's not downloading for any reason, stick up your hand; we can come around and troubleshoot this for you! Now, once you have the data open in Excel, you can easily create a frequency table following the steps below:



First, you will need to select the PivotTable option. Ensure the A1 cell is selected, then click into the **Insert** tab and click on **pivot table**. 


![](imgs/week2/click_pivot.png)




This will open a pop-up window, where you want to make sure that you select 'New worksheet' when it asks where your pivot table should be placed, and then click OK: 




![](imgs/week2/pivot_popup.png)
...
But how much of all crimes does 'Anti-social behaviour' account for? When we are talking about your variables, we normally want to give details and context so that we can tell a comprehensive and easy-to-understand story with our data. At this stage, we can say that the most frequently occurring crime type (the mode) is anti-social behaviour, with 122,399 incidents recorded by GMP. But *how much* is that? Well, we can introduce another column to our pivot table that tells us more about the **proportion** of all crimes that each crime type accounts for. 

To do this, drag from the top the variable Crime.Type into the values box once more: 

![](imgs/week2/drag_perc_valu.gif)

You will see a third column appear, identical to the second one, with the frequencies. To turn these into per cent values, click on the little downward arrow on the box of the value you just dragged into the values box (**some of these steps may look slightly different if you are working on a Mac**): 


![](imgs/week2/down_arrow_perc.png)


When you click on that downward arrow, a menu will appear. 

![](imgs/week2/value_field_settings.png)

Click on the "Value Field Settings..." (or the `Info` tab on Mac) option to open up a new menu window, where you can select what column you want to display.


![](imgs/week2/vfs_menu.png)


You can pick any of these, and it will turn your column of counts (you can see that the default is set to **Count**) to whatever it is that you selected. In this case, since we had a frequency table, we are looking at the count of each one, so leave this as it is. Instead, click on the tab "Show Values As" (or for Mac, it's "Show Data As").


![](imgs/week2/perc_menu.gif)


Then click on the dropdown menu (initially, it will say 'No Calculation'). Again, you will see a variety of possible options to choose from. Here, we want to select "% of Grand Total". Don't worry about the other options for now; we will address those next week when we make frequency tables with two variables. You can also rename the column using the 'Custom Name' box (or 'Field Name' on Mac). Here, we change the name from 'Count of Crime.type2' to '% of all crimes'. As we discussed last week, it's always better to have descriptive and meaningful variable names. 
...
Does the frequency of any of these crime types surprise you? Is this what you expected? When we speak about recorded crime in such general terms, you have to consider that all these very diverse crime types are included in such an umbrella term. So if you begin to hear about an increase in crime, surely you should begin asking - increase in which crimes? An increase in burglaries is a very different thing from an increase in robberies, no? They would require different responses from the police, for example, and have different effects on people's experiences of victimisation and fear of crime. The response will be dictated by what is increasing and why. We might need more on-street police foot patrols in robbery hotspots, but for burglary, maybe we need more visible burglar alarms or higher fences for security. Therefore, examining the types of crime and their frequencies can indeed lead to some instrumental insight. 
...
### Activity: Bar charts in Excel

OK, so let's make our own bar chart in Excel. 


Go back to your pivot table, which has the frequency of each crime type. Click anywhere in the table to highlight it all. However, you might not want all the values. You can see that in the last row, we have a total column. If we're comparing the crime types against each other, it might not make sense to also include a bar for the total. We probably don't want to include our new percentage column either. So, you might want to only highlight the categories and count columns, like this:

![](imgs/week2/crime_type_table.png)

When the pivot table is highlighted, click on the `Insert` tab on the top menu. This should be pretty similar for Mac.

![](imgs/week2/on_pc_insert.png)

Once you click on that, you will see a whole menu of possible charts appear. For Mac, you might have to look further to the right and find the small image of a barplot.

![](imgs/week2/mac_barplot.png)

Look for the one that says 'Column' (or '2D Column') and then select 'Clustered Column'. 

And that's it! Once you click on that, a chart will appear! Yay!


![](imgs/week2/chart_appears.png)



Now, you can stylise your graph. First, you might want to arrange crime.type in an order from most to least frequent rather than alphabetical order. To do this, you must sort the data in the table. 
...
Also, in some versions of Excel, you need to highlight only one cell in the column you want to sort on, and there is no arrow next to the sort button. Instead, you click on the button, and a pop-up window will appear. You can choose 'Descending' on this pop-up window: 

![](imgs/week2/sort_graph_newvs.png)


You can also stylise your graph to make it look the way you like. As we mentioned, we will go through some theory behind data visualisation, but if you want to spend some time making your graphs nice now, then below are some links you might find helpful: 
...
Note that the column with the number of crimes in it might initially be labelled by your pivot table as something else; for example, it could be labelled as a count of borough (as it counts the occurrence of each borough and might not automatically realise that each row/ observation is one crime). So feel free to rename this column by simply clicking in the cell and writing "number of crimes". 
...
You will often hear numeric variables summarised by the measure of central tendency. These are the **mean** and the **median**. You will have encountered the mean before, but possibly referred to as the **average**. In statistical language, you will hear people talk about *the mean number of crimes per borough is `r I(as.character(mean(per_borough_crimes$number.of.crimes)))` crimes*. More commonly, you might hear people refer to this as the _average_ number of crimes per borough, which is of course therefore also `r I(as.character(mean(per_borough_crimes$number.of.crimes)))` crimes.
...
Woohoo! The mean of the middle two numbers, which is the median, is `r I(as.character(sum(34619 + 35122)/2))`. And this gives us our second measure of central tendency. We can see that this number is actually quite far off from our mean number of crimes per borough. This means that our data is **skewed** by our outlier! When our data is **not skewed**, our mean and our median should be the *same value* (or at least very close to one another). The more different they are, the greater the skew in our data! We will talk about skew and things like the normal distribution a bit later and more in your next term, but you should have a basic understanding of the difference between the mean and the median, what this difference means, and when each one is appropriate to use. 
...
### Activity: Mean and median in Excel
...
The formula for calculating the mean (which is commonly used as the statistical term for the average) is simply:


`=AVERAGE()`


Inside the brackets, you have to enter *what* it is that you want to calculate the average of. Remember, you can either type in the cells you want to include manually or highlight them by clicking on them and then press Enter. 
...
The above is from an [article from the website fivethirttyeight](https://fivethirtyeight.com/features/al-gores-new-movie-exposes-the-big-flaw-in-online-movie-ratings/). It points out that the average IMDB rating for this film, which is 5.2, actually masks what is interesting about this film - the extent to which it polarises people. 


We spoke about the measures of central tendency above and how they can be effective summaries of data, but can also mask some important information. This is a good example of that. Let's consider 6 films from 2017 with an IMDB rating of 5.2. This means that the average of all the ratings from all the people who have seen the film and then scored it on IMDB. These are: 
...
This graph is called a **histogram**. While it may at first glance resemble a bar chart, it is subtly different. If it doesn't quite make sense, have a look at 
[this interactive essay on histograms](http://tinlizzie.org/histograms/). Even if you are very confident with histograms, we would recommend that you take time to go through this interactive tutorial. It gives you a really great, hands-on experience in building one. The [Chris Wild video we linked earlier](https://www.youtube.com/watch?v=U3lk2nQYfAQ&amp=&list=PL8CRAVedURQrlxeFfme0TEgaj1_h67JUR&amp=&index=4) also shows you about histograms. They are excellent for plotting the distribution of numeric data. But how do they work? Well, we are really hoping that you have gone through the video and the tutorial, but we also just want to reinforce your learning and explain here. 


Let's say we have some numeric data. We know already that we can't consider each raw numeric value to be a category, which is why we can't build a frequency table. Every entry would only appear once. But what we *can* do is create **bins** for our numeric data to fall into. Think of your numeric variable along the horizontal x-axis. Something like this: 

![](imgs/week2/hist_blank_x.png)
...
And that's all there is to it. Once you have your bins, you just count how many values fall into each interval. If you think about it, by assigning bins to our numeric variable, we are kind of _creating a categorical variable from our raw numeric variable_.
...
If it does, then that is excellent, and you can move on now. If it does **not**, then please raise your hand, and we will come around and try to clarify this for you. It is completely fine to need further clarification on this topic -- that's what we are here for! But also make sure that before you do this, you go through the resources above, especially [the interactive essay](http://tinlizzie.org/histograms/). 
...
Now that's more like it! We can see here that most of the boroughs fall into the bins that collect boroughs with crimes between 20,000 and 50,000, with our outlier borough of central Manchester far away on the right. Your bin width will always be related to your unit of measurement. If we were talking about the number of crimes a day, this would be a very different story because we'll have smaller numbers. 
...
![](imgs/week2/bin_col.png)


The other thing you will need, besides deciding on some bins, is to use the data analysis toolpak. You will have installed this in the lab session last week. **If you could not install the data analysis toolpak in week one, you can scroll down to the section 'Histograms in Excel without the data analysis toolpak'.**
...
#### Activity: Histograms in Excel without the data analysis toolpak
...
First, you will need to select the resulting pivot table with the names of boroughs and counts of crimes, and copy and paste this table into a new spreadsheet. You can do this by selecting the table, clicking Ctrl+C/ Command+C, opening a new spreadsheet, and copying the values (without format) into it. Like this:


![](imgs/week2/copy_paste.png)

The resulting table should look like this:


![](imgs/week2/result_paste.png)


Once this is done, we will need to manually add one small change to our data. We are not interested in including the 'Grand total' in our histogram (it is not one of our units of analysis, but the sum of the totals across observations). However, we will need to tell Excel the minimum value that should be considered in the histogram. Since the number of crimes is a ratio variable (remember? this is the level of measurement of our variable), the minimum of crimes that can be recorded is... 0. We will remove 'Grand total' and '420082', and add 'Minimum' and '0' instead in our dataset.


![](imgs/week2/minumum_0.png)


Now, we can select all values in the 'Count of borough' column and create our histogram.


![](imgs/week2/select_column.png)

From within the 'Insert' menu, find the 'Insert static chart' image, and select the 'Histogram' option:


![](imgs/week2/static_chart_arrow.png)


![](imgs/week2/histogram.png)
...
![](imgs/quartile_borough1.png)

We can easily identify the minimum and the maximum values, right?

And we can also quite easily draw the median:

![](imgs/quartile_borough2.png)

Now the quartiles are just the middle of the two halves of the data. Like so: 


![](imgs/quartile_borough3.png)
...
In the table, with our numerical results, we saw the lower quartile for the number of crimes (or the 25th percentile) is 33,463 (25% of the boroughs have fewer crimes than that), and the upper quartile (or 75% percentile) was 39,312 (about 25% of the boroughs have more crimes than that). The interquartile range then is `r I(39312-33463)` (`upper quartile - lower quartile = 39312-33463 = ``r I(39312-33463)`).


Because the interquartile range excludes everybody with the lowest and the highest values in the distribution (we are only looking at the difference between the lower and the upper quartile), it is a measure of spread that is less sensitive to outliers (extremely atypical, very high or very low values). 
...
A more powerful approach for measuring spread uses the **standard deviation**. What is the standard deviation? This is the most complex concept we have covered so far, and by now, you may be a bit tired and overwhelmed with all we have covered today; thus, do not worry too much if you get a bit confused. We will also come back to it next week and remember that you can also re-watch the videos for this week for a verbal explanation.

One way of thinking about spread is to examine how far each data value is from the mean. This difference is called a deviation. If we look at our dataset, we see that Trafford recorded 22,587 crimes. If the mean for all the boroughs is 42,008, then Trafford has a deviation of 19,421 (i.e., 42,008 - 22,587).
...
Conceptually understanding the variance and the standard deviation will possibly take you a while. We will come back to it, so don’t worry too much if you feel a bit confused right now. Remember that you can ask questions during the in-person classes and post on the Canvas discussion board. Please also make sure you read the recommended textbooks. 
...
When your standard deviation is larger than the mean, it signifies that you have a skewed distribution. The larger the difference is, the greater the skew. In fact, the use of the standard deviation in such cases is a sign to conclude that you are dealing with a skewed distribution. 


### Activity: Standard Deviation in Excel
...
The structure and content of your report will always depend on what the purpose of the report is and who the readership will be. For example, if you were working as an analyst for a police force, you would want to consider who will be reading the report. A report for your analyst colleagues might look very different from one that will be read by active police officers. Remember, all these skills are useful in other sectors too. You know your Tesco Club Card? There are whole teams of Tesco analysts beavering away analysing the (anonymised) data collected by all of our Club Cards. And guess what? They will use all the skills you've learnt today and write reports for colleagues and managers to summarise their findings. This is a handy and highly applicable skill!

The structure of reports is important too. For the assignment of this class, you will also be given specific guidelines that you will have to adhere to, both in terms of content and format. But these provide just a skeleton, which you populate with your own research, findings, and interpretations of your data. So, while there are guidelines and templates, every report is unique. 
...
You also want to quickly discuss your data. Where does it come from? What does it represent? We will talk more about the details and importance of concepts such as sampling, research design, and other relevant issues in future weeks. For now, we will just acknowledge the source of our data. 
