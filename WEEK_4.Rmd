...
<details>
<summary><i>Alternative to get the average for each sample!</i></summary>
...
<details>
<summary><i>DOing the above using the IF function</i></summary>
...
We can create a mini table with our codes and values in the data. Make sure the formatting follows exactly as in the original table. Sometimes, Excel will give you an error if you try to match something where a word is capitalised in one table but not in the other one. Something like this:
...
Instead, we will learn a new function in Excel, one which you will often use, called `VLOOKUP()`. This function uses a lookup table to assign variables to a new column. In this case, our lookup table, which we created, is located at the bottom. It tells Excel that, in this case, Very safe is 1, Fairly safe is 2, A bit unsafe is 3, and Very unsafe is 4. We can use this to create our new, re-coded variables. 
